...
export default function Home() {
  return <main>
    ...
    <section id="work" className="work shell"><div className="section-head"><p className="label">Selected work</p><h2>Less theatre.<br />More <i>working software.</i></h2></div><div className="project-list">{projects.map((project) => <Link key={project.title} href={project.href} className={`project ${project.tone}`}><span className="project-index">{project.index}</span><div><p>{project.eyebrow}</p><h3>{project.title}</h3><span>{project.line}</span></div><b aria-hidden="true">↗</b></Link>)}</div></section>
